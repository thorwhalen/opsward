...
def generate(scan_result: ScanResult) -> list[GeneratedFile]:
    ...
    if not template_target.exists():
        files.append(_render(
            'shared/decisions/0000-template.md', template_target, variables,
        ))
    ...
    if 'diagnose-setup' not in existing_skills:
        target = (
            sr.project_root / '.claude' / 'skills' / 'diagnose-setup' / 'SKILL.md'
        )
        files.append(_render(
            'shared/diagnose-setup/SKILL.md', target, variables,
        ))

    # maintain-docs skill
    if 'maintain-docs' not in existing_skills:
        target = (
            sr.project_root / '.claude' / 'skills' / 'maintain-docs' / 'SKILL.md'
        )
        files.append(_render(
            'shared/maintain-docs/SKILL.md', target, variables,
        ))
